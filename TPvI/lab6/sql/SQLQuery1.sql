...
CREATE TABLE CITIZENCOUNT(
	IDtype int not null CONSTRAINT FK_IDtype foreign key references CITIZENTYPES(IDtype),
	Amount int,
	IDcity int not null CONSTRAINT FK_IDcitycount foreign key references CITIES(IDcity)
);

INSERT into CITIES(IDcity, Name, FoundationYear, Area)
	values(1, 'Минск', 1067, 348),
		  (2, 'Одесса', 1415, 236.6),
		  (3, 'Москва', 1147, 2561),
		  (4, 'Рига', 1201, 307),
		  (5, 'Варшава', 1313, 517)
...
INSERT into CITIZENTYPES(IDtype, Name, Language)
	values(1, 'белорус', 'белорусский'),
		  (2, 'россиянин', 'русский'),
		  (3, 'украинец', 'украинский'),
		  (4, 'поляк', 'польский'),
		  (5, 'латыш', 'латышский'),
		  (6, 'еврей', 'иврит'),
		  (7, 'армян', 'армянский')
...
FROM CITIES 
	...
	WHERE CITIZENTYPES.Language LIKE 'русский'

SELECT CITIES.Name, CITIES.FoundationYear, CITIES.Area, CITIZENCOUNT.Amount
FROM CITIES 
	JOIN CITIZENCOUNT ON CITIES.IDcity = CITIZENCOUNT.IDcity
	JOIN CITIZENTYPES ON CITIZENTYPES.IDtype = CITIZENCOUNT.IDtype
	WHERE CITIZENTYPES.Name LIKE 'белорус'

SELECT TOP 1 CITIES.Name, CITIZENTYPES.Name, CITIZENTYPES.Language, CITIZENCOUNT.Amount
FROM CITIES 
	JOIN CITIZENCOUNT ON CITIES.IDcity = CITIZENCOUNT.IDcity
	JOIN CITIZENTYPES ON CITIZENTYPES.IDtype = CITIZENCOUNT.IDtype
