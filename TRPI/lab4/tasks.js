...
//     value: T
// }
function createIterator(products) {
    let keysOfCategories = Object.keys(products.Shoes);
    ...
    return {
        next: function () {
            ...
            let currentCategoryName = keysOfCategories[currentCategory];
            let keyTyped = currentCategoryName;
            let currentCategoryValue = products.Shoes[keyTyped];
            if (currentProduct >= currentCategoryValue.length) {
                currentCategory++;
                currentProduct = 0;
                return this.next();
            }
            let currentProductValue = currentCategoryValue[currentProduct];
            currentProduct++;
            return {
                done: false,
...
function FilterGoods(products, min, max, size, color) {
    let filtered = [];
    console.log('\n\n');
    let keys = Object.keys(products.Shoes);
    for (let key of keys) {
        let keyTyped = key;
        let shoes = products.Shoes[keyTyped];
        shoes.forEach(shoe => {
...
function addShoes(products, category, newShoes) {
